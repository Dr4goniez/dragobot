...
const escapedPpSubpages = Array.from(pp).map((el) => mwbot_1.Util.escapeRegExp(el + '/'));
/**
 ...
 * @returns
 */
async function removePp(quitBefore) {
    ...
    const unprotectedPages = (0, lib_1.filterSet)(transclusions, (page) => !protectedPages.has(page));
    ...
    for (const page of unprotectedPages) {
        ...
        const result = await mwbot.edit(page, createTransformationPredicate(page)).catch((err) => err);
 ...
 */
async function getPpTransclusions() {
    const mwbot = (0, mwbot_1.getMwbot)();
    const batches = [];
    for (const temp of pp) {
        batches.push(mwbot.continuedRequest({
            titles: temp,
            prop: 'transcludedin',
            tiprop: 'title',
            tilimit: 'max'
 ...
 * *This function never rejects.*
 */
async function filterProtected(pages) {
 ...
 * @returns
 */
function isProtected(protectionArray, now) {
