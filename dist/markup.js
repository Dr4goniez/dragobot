 ...
 */
async function markupPage(page, checkGlobal) {
    console.log(`Checking ${page}...`);
    const res = await (0, mwbot_1.getMwbot)().edit(page, createTransformationPredicate(page, checkGlobal)).catch((err) => err);
 ...
 */
function createTransformationPredicate(page, checkGlobal) {
    return async (wikitext) => {
        ...
        const sanitizeUsername = (username) => {
            if (/^[\u10A0-\u10FF]/.test(username)) {
                ...
                return mwbot.Title.phpCharToUpper(mwbot_ts_1.Mwbot.String.charAt(username, 0)) + username.slice(1);
            }
        };
        ...
        const templateMap = wikitext.parseTemplates({
            ...
            if (!mwbot.Template.is(temp, 'ParsedTemplate') ||
            ...
            const ip = ip_wiki_1.IP.newFromText(user);
        ...
        if (mwbot_1.Util.isEmptyObject(templateMap)) {
        ...
        Object.entries(templateMap).forEach(([key, obj]) => {
            ...
            else if (user instanceof ip_wiki_1.IP) {
        ...
        const remainingIps = (0, lib_1.filterSet)(ips, (ip) => !(ip in blockInfo));
        ...
        Object.values(templateMap).forEach((temp) => {
            ...
            if (isNewBlock) {
                ...
                if (temp.user instanceof ip_wiki_1.IP && !block.anononly) {
                    flags.push('ハードブロック');
                }
                const bitLen = !(temp.user instanceof ip_wiki_1.IP) ? '' : (() => {
                    const cidr = ip_wiki_1.IP.newFromText(block.user); // This is always an IP instance
        ...
        let reblockInfo;
        if (mwbot_1.Util.isEmptyObject(reblockMap) === false && (reblockInfo = await checkReblocks(reblockMap))) {
            for (const username in reblockMap) {
                ...
                const key = Object.entries(templateMap).find(([_, obj]) => obj.user instanceof ip_wiki_1.IP ? obj.user.equals(username) : obj.user === username)?.[0];
                if (!key) {
        ...
        do { // Create a break-able block to prevent deep nests
            ...
            Object.values(templateMap).forEach(({ user, date, hasBotTimestamp }) => {
                ...
                if (user instanceof ip_wiki_1.IP) {
            ...
            if (gUsers.size && (lockedUsers = await queryLockedUsers(gUsers)).size) {
                ...
                gUsers = (0, lib_1.filterSet)(gUsers, (username) => !lockedUsers.has(username));
                if (!gUsers.size && !gIps.size) {
                    break;
                }
            }
            ...
            gIps = (0, lib_1.filterSet)(gIps, (ip) => !(ip in gblockInfo));
            if (gIps.size) {
                await queryGloballyBlockedIps(gblockInfo, gIps);
            }
            if (mwbot_1.Util.isEmptyObject(gblockInfo)) {
                break;
            }
            ...
            Object.values(templateMap).forEach((temp) => {
                ...
                if (isNewBlock) {
                    const bitLen = !(temp.user instanceof ip_wiki_1.IP) ? '' : (() => {
                        const cidr = ip_wiki_1.IP.newFromText(gblock.target); // This is always an IP instance
                        if (cidr && cidr.isCIDR() && cidr.equals(temp.user) === false) {
                            return `/${cidr.getBitLength()}で`;
                        }
                        return '';
...
async function updateLogids() {
    ...
    const response = await (0, mwbot_1.getMwbot)().get(params).catch((err) => {
...
async function updateDiffids() {
    ...
    const response = await (0, mwbot_1.getMwbot)().massRequest(params, 'revids');
 ...
 * @returns *This function never rejects*.
 */
async function processRemainingLogids() {
 ...
 * *This function never rejects*.
 */
async function scrapeUsernameByLogid(logid) {
    const url = 'https://ja.wikipedia.org/w/index.php?title=Special:Log&logid=' + logid;
    const $ = await (0, lib_1.scrapeWebpage)(url);
    if (!$)
        return null;
 ...
 * @returns
 */
async function queryBlockedUsers(users, ips, isANS) {
    const params = {
        list: 'blocks',
    ...
    const response = await (0, mwbot_1.getMwbot)().massRequest(params, 'bkusers');
 ...
 * @returns
 */
async function queryBlockedIps(info, ips) {
    const params = {
        list: 'blocks',
        bkprop: 'user|timestamp|expiry|restrictions|flags',
        bklimit: 'max',
    ...
    const response = await (0, mwbot_1.getMwbot)().massRequest(params, 'bkip', 1);
    const ipIter = ips.values();
    for (const res of response) {
        const ip = ipIter.next().value;
        if (res instanceof Error) {
            console.dir(res, { depth: 3 });
            continue;
        }
        ...
        const blockInfo = resBlocks.filter((obj) => !obj.automatic && obj.user);
 ...
 */
function getNarrowestBlockIndex(blocksArr, refDate) {
    let idx = null;
    let narrowestSubnet;
 ...
 */
function getBlockDuration(blockTimestamp, expiryTimestamp) {
 ...
 * @returns
 */
function formatDateToMDinGMT9(date) {
 ...
 * or `null` if not.
 */
async function checkReblocks(reblockMap) {
    const users = Object.keys(reblockMap);
    const response = await (0, mwbot_1.getMwbot)().massRequest({
        list: 'logevents',
        letype: 'block',
    ...
    for (let i = 0; i < response.length; i++) {
        ...
        const isAfterReferenceDate = (timestamp) => {
    ...
    return !mwbot_1.Util.isEmptyObject(ret) ? ret : null;
    function sanitizeParams(blockLog) {
        const params = blockLog.params; // Already ensured this property is present
        return {
            duration: params.duration, // Always present
            expiry: params.expiry, // Missing if `duration` is `'infinity'`
            flags: (params.flags || []),
            sitewide: !!params.sitewide,
            restrictions: params.restrictions,
            timestamp: blockLog.timestamp, // Already verified presence
        };
    }
}
/**
 ...
 * @param rest2
 */
function restrictionsDiffer(rest1, rest2) {
    // If one is defined and the other is not
    if (!rest1 && rest2 || rest1 && !rest2)
        return true;
    ...
    if (typeof pages1 !== typeof pages2 || (pages1 && pages2 && !mwbot_1.Util.arraysEqual(pages1.map(p => p.page_title), pages2.map(p => p.page_title), true))) {
        return true;
    }
    if (typeof namespaces1 !== typeof namespaces2 ||
        (namespaces1 && namespaces2 && !mwbot_1.Util.arraysEqual(namespaces1, namespaces2, true))) {
        return true;
    }
    if (typeof actions1 !== typeof actions2 ||
        (actions1 && actions2 && !mwbot_1.Util.arraysEqual(actions1, actions2, true))) {
        return true;
    }
    return false;
}
 ...
 * @returns *This function never rejects*.
 */
async function queryLockedUsers(users) {
    ...
    const response = await (0, mwbot_1.getMwbot)().massRequest(params, ['agufrom', 'aguto'], 1);
 ...
 * @returns
 */
async function queryGloballyBlockedUsers(users, ips, isANS) {
    const params = {
        list: 'globalblocks',
        bgtargets: [...users, ...ips],
    ...
    const response = await (0, mwbot_1.getMwbot)().massRequest(params, 'bgtargets');
    return response.reduce((acc, res) => {
        if (res instanceof Error) {
            console.dir(res, { depth: 3 });
            return acc;
 ...
 * @returns
 */
async function queryGloballyBlockedIps(info, ips) {
    const params = {
        list: 'globalblocks',
        bgip: [...ips],
        bgprop: 'target|timestamp|expiry',
    ...
    const response = await (0, mwbot_1.getMwbot)().massRequest(params, 'bgip', 1);
    const ipsIter = ips.values();
    for (const res of response) {
        const ip = ipsIter.next().value;
        if (res instanceof Error) {
            console.dir(res, { depth: 3 });
            continue;
        }
        ...
        const gblockInfo = resGblocks.filter(({ automatic, target, timestamp, expiry }) => !automatic && target && timestamp && expiry);
 ...
 * @returns
 */
function generateSummaryUserLink(info) {
    const status = info.reblocked || info.domain + info.duration;
    const username = info.user instanceof ip_wiki_1.IP ? info.user.abbreviate() : info.user;
