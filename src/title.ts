 ...
 */
const wgNamespaceIds: {[alias: string]: number;} = {
	'メディア': -2,
	'特別': -1,
	'': 0,
	'ノート': 1,
 ...
 */
const wgFormattedNamespaces: {[ns: string]: string;} = {
	'0': '',
	'1': 'ノート',
	'2': '利用者',
	'3': '利用者‐会話',
 ...
 */
const wgCaseSensitiveNamespaces: number[] = [2300, 2301, 2302, 2303];

/**
 * The IDs of the namespaces in which signature buttons should be shown, in addition to talk namespaces.
 */
const wgExtraSignatureNamespaces: number[] = [4, 12];
...
type SanitationRuleKeys = (
	|'pattern'
	|'replace'
	|'generalRule'
	|'fileRule'
);
const sanitationRules: SanitationRule[] = [
	// "signature"
	{
		pattern: /~{3}/g,
		replace: '',
...
export class Title {
	 ...
	 */
	constructor(title: string, namespace?: number, parsed?: TitleProperties) {
	 ...
	 * @returns 
	 */
	private static parse(title: string, ns?: number): TitleProperties|null {
		...
		if (namespace !== nsSpecial && byteLength(title) > titleMaxBytes) {
			return null;
		}
	 ...
	 */
	static newFromUserInput(title: string, namespace?: number, options?: {forUploading: boolean;}) {
	 ...
	 */
	static isTalkNamespace(namespace: number): boolean {
	 ...
	 */
	static wantSignaturesNamespace(namespace: number): boolean {
	...
	static exist: Exist = {
		pages: {},
		set: (titles: string|string[], state = true) => {
	...
	static normalizeExtension(extension: string): string {
		...
		const normalizations: {[ext: string]: string;} = {
			htm: 'html',
			jpeg: 'jpg',
			mpeg: 'mpg',
			tiff: 'tif',
	...
	*/
	toString = this.getPrefixedDb;

	/**
	* Alias of Title#getPrefixedText
	*/
	toText = this.getPrefixedText;

	/**
	* Check pagetitle equality.
	* @original
	*/
	equals(title: string|Title): boolean {
	...
	* @original
	*/
	equalsToAny(titles: (string|Title)[]): boolean {
	...
	* @original
	*/
	equalsToAll(titles: (string|Title)[]): boolean {
 ...
 */
export function getNsIdByName(alias: string): number|null {
 ...
 */
export function getNsIdsByType(type: 'main'|'talk'): number[] {
	return Object.keys(wgFormattedNamespaces).reduce((acc: number[], ns: string) => {
 ...
 */
export function isKnownNamespace(namespace: number): boolean {
 ...
 */
export function getNamespacePrefix(namespace: number): string {
 ...
 */
function sanitize(s: string, filter: SanitationRuleKeys[]) {
 ...
 */
function trimToByteLength(s: string, length: number): string {
 ...
 */
function trimFileNameToByteLength(name: string, extension: string): string {
